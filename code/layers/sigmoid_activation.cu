...
__global__ void sigmoidActivationBackprop(float* Z, float* dA, float* dZ,
	...
	if (index < Z_x_dim * Z_y_dim) { // 保持边界检查
		float s_val = sigmoid(Z[index]);
		dZ[index] = dA[index] * s_val * (1.0f - s_val);
	}
}
...
Matrix& SigmoidActivation::backprop(Matrix& dA) {
...
void SigmoidActivation::get_learnable_parameters(ParameterGroup& group)
{
}
